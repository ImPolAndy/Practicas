...
template<typename T>
void Lista<T>::search(T datos)
{
    Node<T> *temp = m_inicial;
    int cont = 1;
    int cont2 = 0;
...
template<typename T>
void Lista<T>::MostarLista(Lista<string> &sLista)
{
    Lista<int>::iterator pos;
    pos=sLista.agregaInicio();
    while(pos!=sLista.agregaFinal())
    {
        cout<<*pos<<endl;
        pos++;
    }
